"""
Canal de Denuncias — canal_denuncias.py
Router FastAPI · SQLite · Ley 2/2023 (España) / Ley 27.401 (Argentina)
Ph.D. Vicente H. Monteverde · Ecosistema Transparencia

Sistema de PINs con expiración:
  - PIN_CONSULTOR: definido en .env — acceso total
  - PIN_COMPLIANCE: definido en .env — acceso total
  - Ambos PINs tienen fecha de expiración configurable
...
# ── Configuración de PINs desde .env ────────────────────────────────────────
# En .env agregar:
#   PIN_CONSULTOR=1234
#   PIN_COMPLIANCE=5678
#   PIN_EXPIRA_DIAS=30        (el PIN en sí expira cada N días, default 30)
#   SESSION_HOURS=8           (cada sesión dura N horas, default 8)
...
def _pin_expirado() -> bool:
    """Verifica si los PINs actuales superaron su vida útil."""
    # Usamos el archivo .env mtime como referencia de cuándo se configuraron
    # Si no existe referencia, asumimos que son válidos (primera vez)
...
def _nueva_sesion(rol: str) -> str:
    """Genera un token de sesión con expiración."""
...
def _validar_token(token: str) -> dict:
    """Valida el token y devuelve {rol}. Lanza 401 si inválido o expirado."""
    if not token:
        raise HTTPException(status_code=401, detail="Token requerido. Iniciá sesión primero.")
    sesion = _sesiones.get(token)
    if not sesion:
        raise HTTPException(status_code=401, detail="Sesión inválida o expirada.")
    if datetime.utcnow() > sesion["expira"]:
        del _sesiones[token]
        raise HTTPException(status_code=401, detail="Sesión expirada. Volvé a ingresar el PIN.")
    return sesion


def _log_sesion(rol: str, accion: str):
    """Registra accesos en la BD para auditoría."""
    try:
        conn = get_conn()
        conn.execute(
...
def login(body: LoginRequest):
    ...
    if _pin_expirado():
        raise HTTPException(
            status_code=403,
            detail=f"Los PINs expiraron. Actualizá PIN_CONSULTOR y PIN_COMPLIANCE en el .env y reiniciá el servidor."
        )
...
@router.post("/denuncias/auth/logout", summary="Cerrar sesión")
...
@router.get("/denuncias/auth/estado", summary="Verificar estado de sesión")
...
@router.post("/denuncias", summary="Crear denuncia (público)")
def crear_denuncia(body: DenunciaCreate):
    """Crea una denuncia. No requiere autenticación."""
    if not body.es_anonima and not body.datos_contacto:
        raise HTTPException(
            status_code=422,
            detail="datos_contacto es obligatorio cuando la denuncia no es anónima."
        )
    ...
    return {
        ...
        "aviso": "Guarde este código. Es la única forma de consultar su caso."
    }


@router.get("/denuncias/seguimiento/{codigo}", summary="Consultar caso por código (público)")
def consultar_seguimiento(codigo: str):
    """El denunciante consulta el estado de su caso con el código de seguimiento."""
    conn = get_conn()
    try:
        row = conn.execute(
...
@router.post("/denuncias/seguimiento/{codigo}/mensaje", summary="Agregar mensaje al caso (público)")
def agregar_mensaje_seguimiento(codigo: str, body: MensajeSeguimiento):
    """El denunciante agrega información al caso."""
...
# ── Endpoints de gestión — REQUIEREN TOKEN ───────────────────────────────────

@router.get("/denuncias", summary="Listar todas las denuncias (panel — requiere PIN)")
def listar_denuncias(
    ...
    """Panel de gestión. Requiere token de sesión en header X-Session-Token."""
    sesion = _validar_token(x_session_token)
...
@router.patch("/denuncias/{id}", summary="Actualizar estado/prioridad (panel — requiere PIN)")
def actualizar_denuncia(
    id: int,
    body: DenunciaUpdate,
    x_session_token: str = Header(...),
):
    """Cambia estado, prioridad o área. Requiere token."""
    sesion = _validar_token(x_session_token)
...
@router.get("/denuncias/alertas", summary="Alertas activas (panel — requiere PIN)")
def listar_alertas(
...
@router.patch("/denuncias/alertas/{id}/leer", summary="Marcar alerta como leída (requiere PIN)")
def marcar_alerta_leida(id: int, x_session_token: str = Header(...)):
    ...
    try:
        ...
        conn.close()
    return {"mensaje": "Alerta marcada como leída.", "id": id}


@router.get("/denuncias/resumen/por-area", summary="Resumen por área (requiere PIN)")
